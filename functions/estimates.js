...
// functions/api/estimates.ts
init_schema2();
init_lib();
async function createEstimate(db, estimateData) {
  const { estimates: estimates2 } = await Promise.resolve().then(() => (init_schema2(), schema_exports));
  const [estimate] = await db.insert(estimates2).values(estimateData).returning();
  return estimate;
}
async function getEstimates(db) {
  const { estimates: estimates2 } = await Promise.resolve().then(() => (init_schema2(), schema_exports));
  const allEstimates = await db.select().from(estimates2);
  return allEstimates;
}
...
async function onRequestGet(context) {
  try {
    const db = createDB(context.env);
    const estimates2 = await getEstimates(db);
    return Response.json(estimates2);
  } catch (error) {
