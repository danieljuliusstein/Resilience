...
var PgSelectBuilder, PgSelectQueryBuilderBase, PgSelectBase, getPgSetOperators, union2, unionAll2, intersect2, intersectAll, except2, exceptAll;
...
var RelationalQueryBuilder2, PgRelationalQuery;
...
// server/email-octopus.ts
var email_octopus_exports = {};
__export(email_octopus_exports, {
  EmailOctopusService: () => EmailOctopusService
});
var EmailOctopusService;
var init_email_octopus = __esm({
  "server/email-octopus.ts"() {
    "use strict";
    EmailOctopusService = class {
      apiKey;
      apiUrl = "https://emailoctopus.com/api/1.6";
      adminEmail;
      fromEmail;
      constructor(env) {
      ...
      async createSingleRecipientCampaign(emailData) {
        try {
      ...
      async addRecipientToCampaign(campaignId, email) {
        try {
      ...
      async addToList(listId, subscriberData) {
        try {
      ...
      async sendToList(listId, emailData) {
        try {
          const response = await fetch(`${this.apiUrl}/campaigns`, {
            method: "POST",
            headers: {
...
// server/email-drip-octopus.ts
var email_drip_octopus_exports = {};
__export(email_drip_octopus_exports, {
  EmailScheduler: () => EmailScheduler,
  createDripCampaign: () => createDripCampaign,
  dripCampaignTemplates: () => dripCampaignTemplates,
  scheduleEmailDripCampaign: () => scheduleEmailDripCampaign,
  sendWelcomeEmail: () => sendWelcomeEmail
});
async function scheduleEmailDripCampaign(subscriberData, env) {
  try {
...
async function createDripCampaign(campaignName, listId, template, env) {
  try {
    const emailService = new EmailOctopusService(env);
...
async function sendWelcomeEmail(subscriberData, env) {
  try {
    const emailService = new EmailOctopusService(env);
    const personalizedContent = dripCampaignTemplates.welcome.htmlContent.replace(/{{FirstName}}/g, subscriberData.firstName || "there");
...
var dripCampaignTemplates, EmailScheduler;
var init_email_drip_octopus = __esm({
  "server/email-drip-octopus.ts"() {
    "use strict";
    init_email_octopus();
    ...
    EmailScheduler = class {
      ...
      async scheduleEmail(subscriberData, template, delayHours = 24) {
        try {
          const scheduleId = crypto.randomUUID();
          const scheduledTime = Date.now() + delayHours * 60 * 60 * 1e3;
          const scheduleData = {
            subscriberData,
            template,
            scheduledTime,
          ...
          await this.env.KV.put(`schedule:${scheduleId}`, JSON.stringify(scheduleData), {
            expirationTtl: delayHours * 60 * 60 + 3600
            // Add 1 hour buffer
          });
...
var NeonSession = class _NeonSession extends PgSession {
  ...
  async transaction(transaction, config = {}) {
    const session = this.client instanceof eo ? new _NeonSession(await this.client.connect(), this.dialect, this.schema, this.options) : this;
...
function createPostgreSQLDB(databaseUrl) {
  if (!databaseUrl) {
    throw new Error("DATABASE_URL must be set for PostgreSQL connection");
  }
  const client = new eo({ connectionString: databaseUrl });
...
init_schema2();
init_lib();
async function createLead(db, leadData) {
  const { leads: leads2 } = await Promise.resolve().then(() => (init_schema2(), schema_exports));
  const [lead] = await db.insert(leads2).values(leadData).returning();
  return lead;
}
async function getLeads(db) {
  const { leads: leads2 } = await Promise.resolve().then(() => (init_schema2(), schema_exports));
  const allLeads = await db.select().from(leads2);
  return allLeads;
}
async function onRequestPost(context) {
  try {
    ...
    try {
      const { EmailOctopusService: EmailOctopusService2 } = await Promise.resolve().then(() => (init_email_octopus(), email_octopus_exports));
      const emailService = new EmailOctopusService2(context.env);
      await emailService.sendQuoteNotification(leadData);
    } catch (error) {
      console.error("Failed to send email notification:", error);
    }
    try {
      const { scheduleEmailDripCampaign: scheduleEmailDripCampaign2 } = await Promise.resolve().then(() => (init_email_drip_octopus(), email_drip_octopus_exports));
...
async function onRequestGet(context) {
  try {
    const db = createDB(context.env);
    const leads2 = await getLeads(db);
    return Response.json(leads2);
  } catch (error) {
