...
init_schema2();
init_lib();
async function createMessage(db, messageData) {
  const { messages: messages2 } = await Promise.resolve().then(() => (init_schema2(), schema_exports));
  const [message] = await db.insert(messages2).values(messageData).returning();
  return message;
}
async function getMessages(db) {
  const { messages: messages2 } = await Promise.resolve().then(() => (init_schema2(), schema_exports));
  const allMessages = await db.select().from(messages2);
  return allMessages;
}
...
async function onRequestGet(context) {
  try {
    const db = createDB(context.env);
    const messages2 = await getMessages(db);
    return Response.json(messages2);
  } catch (error) {
